...
- Database connections are expensive, so we can have a set of connections open and ready to use, called a connection pool. This is what SQLAlchemy does at client side by `AsyncEngine`.
```
...
Transaction 4: App session B uses DB connection #3  → A's txn 3 ended, #3 freed, B reuses it
```
- The unit of multiplexing is the transaction, not the request i.e if a request has multiple transactions, they can be multiplexed across different connections in the pool. While request A is **between** transactions (waiting for the next HTTP call from the user, thinking, doing non-DB work) — its connection goes back to the pool and RDS Proxy hands it to request B or C (different users). Request A gets a (possibly different) connection for its next transaction. See that there can be overlaps between transactions of different requests, but they can still be multiplexed as long as they are not active at the same time. The key is that the connection is only used for the duration of a transaction, and then it goes back to the pool for other transactions to use.
```
Time →
...
DB conn #7: [==A txn1==][==B txn1==]                                        [==B txn2==]
DB conn #3:                    [==A txn2==]                                        [==A txn3==]
